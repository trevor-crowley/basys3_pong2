...
typedef struct {
    ...
    XHud_gen_uio_map maps[ MAX_UIO_MAPS ];
} XHud_gen_uio_info;

/***************** Variable Definitions **************************************/
static XHud_gen_uio_info uio_info;
...
static int uio_info_read_name(XHud_gen_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XHud_gen_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XHud_gen_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XHud_gen_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XHud_gen_Initialize(XHud_gen *InstancePtr, const char* InstanceName) {
	XHud_gen_uio_info *InfoPtr = &uio_info;
